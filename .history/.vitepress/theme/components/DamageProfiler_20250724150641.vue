<template>
  <div class="analysis-suite-card">
    ...
    <!-- Tab 切换按钮 -->
    <div class="tabs">
      <button :class="{ active: activeTab === 'profiler' }" @click="activeTab = 'profiler'">
        实时验算器
      </button>
...
function renderHeatmap() {
    ...
    heatmapInstance = new Chart(heatmapCanvas.value.getContext('2d'), {
        type: 'matrix',
        data: { datasets: [{
            label: '基础攻击占比 (%)', data: generateHeatmapData(),
...
// 监视Tab切换，只有在第一次切换到Heatmap时才渲染，避免不必要的性能消耗
watch(activeTab, (newTab) => {
    if (newTab === 'heatmap' && !isHeatmapRendered) {
        nextTick(() => {
            renderHeatmap();
        });
    }
