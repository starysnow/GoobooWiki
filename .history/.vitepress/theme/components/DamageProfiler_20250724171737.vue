...
const lineChartRange = reactive({ min: 0, max: 30, step: 1 });
const lineChartData = ref({ labels: [], baseProportions: [], efficiencies: [] });
                ...
                tooltip: {
                    callbacks: {
                        title: function(tooltipItems) {
                            return '基础攻击: ' + parseFloat(tooltipItems[0].label).toFixed(3) + '%';
                        },
                        label: function(context) {
...
function updateLineChart() {
    ...
    lineChartData.value = { labels, baseProportions, efficiencies };
}

// --- 生命周期与监视 (最终版) ---
const debouncedUpdateLineChart = debounce(updateLineChart, 400);
