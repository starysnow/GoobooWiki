        ...
        <div class="modal-content">
          ...
          <div class="table-container">
            <table>
              ...
              <tbody>
                <!-- 使用 processedData 进行遍历 -->
                <tr v-for="(row, rowIndex) in processedData" :key="rowIndex">
                  <template v-for="(cell, colIndex) in row" :key="colIndex">
                    <!-- 使用 v-if, rowspan, colspan -->
                    <td
                      v-if="cell._render"
                      :rowspan="cell._rowspan"
                      :colspan="cell._colspan"
...
// 终极计算属性：全自动识别、计算并处理合并单元格
const processedData = computed(() => {
  if (!props.data || props.data.length === 0) return [];

  // 1. 初始化一个与原始数据同样大小的“元数据网格”
  const metaGrid = props.data.map(row =>
    headers.value.map(header => ({
      value: row[header], // 原始值
      _render: true,      // 是否需要渲染
      _rowspan: 1,        // 占多少行
      _colspan: 1,        // 占多少列
    }))
  ...
  // 2. 从后往前遍历，计算行合并 (rowspan)
  for (let j = 0; j < colCount; j++) { // 遍历每一列
    for (let i = rowCount - 2; i >= 0; i--) { // 从倒数第二行往上
      const currentCell = metaGrid[i][j];
      const belowCell = metaGrid[i + 1][j];

      // 如果当前格与下方格内容相同，且下方格还未被合并
      if (currentCell.value === belowCell.value && belowCell._render) {
        currentCell._rowspan += belowCell._rowspan;
        belowCell._render = false; // 标记下方格不再需要渲染
      }
    }
  }

  // 3. 从后往前遍历，计算列合并 (colspan)
  for (let i = 0; i < rowCount; i++) { // 遍历每一行
    for (let j = colCount - 2; j >= 0; j--) { // 从倒数第二列往左
      const currentCell = metaGrid[i][j];
      const rightCell = metaGrid[i][j + 1];

      // 如果当前格与右方格内容相同，且它们将要合并的行数也相同
      if (currentCell.value === rightCell.value && currentCell._rowspan === rightCell._rowspan && rightCell._render) {
        currentCell._colspan += rightCell._colspan;
        rightCell._render = false; // 标记右方格不再需要渲染
      }
    }
  }
