...
/**
 * 递归地调用 MathJax API 来渲染页面上的所有公式。
 * 它会持续检查 MathJax 是否准备好，直到成功触发渲染。
 */
const renderMath = () => {
  // 检查我们在 config.js 中设置的全局标志
  if (window.mathJaxReady) {
    // 如果标志为 true，说明 MathJax 核心已准备好

    // 确保 MathJax 对象和 typeset 方法都存在
    if (window.MathJax && typeof window.MathJax.typeset === 'function') {
      console.log('MathJax is ready, attempting to typeset...');

      // typeset() 会扫描整个文档并渲染所有找到的公式
      // 如果你的页面非常大且卡顿，可以优化为只渲染当前组件
      // window.MathJax.typeset([container.value]);
      window.MathJax.typeset();
    }
  } else {
    // 如果 MathJax 还没准备好，等待 100 毫秒后再次尝试调用自己
    console.warn('MathJax not ready yet, retrying in 100ms...');
    setTimeout(renderMath, 100);
  }
};

// 在组件首次挂载到 DOM 后，开始尝试渲染
onMounted(() => {
  renderMath();
});

// 在组件因 props 数据变化而更新 DOM 后，再次尝试渲染
onUpdated(() => {
  renderMath();
});

// --- 新增：监听 isFullscreen 的变化 ---
watch(isFullscreen, (newValue) => {
  // 当 isFullscreen 变为 true (即模态框显示时)
  if (newValue) {
    // 使用 nextTick 确保 DOM 已经更新完毕
    // 或者继续使用 setTimeout 也可以
    setTimeout(renderMath, 100);
  }
});
</script>
