...
function jsToReadableFormula(funcNode) {
    ...
    let targetNode = funcNode;

    // 步骤1: 智能提取目标表达式节点
    // 如果是函数，提取其函数体；否则，直接使用该节点。
    if (node.type === 'ObjectMethod' || node.type === 'FunctionExpression' || node.type === 'ArrowFunctionExpression') {
        if (!node.body) return '[空函数体]';
        targetNode = node.body.type === 'BlockStatement' && node.body.body[0]?.type === 'ReturnStatement'
