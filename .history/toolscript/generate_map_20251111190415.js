...
async function generateFlatTranslationMap() {
    ...
    try {
        // 遍历黑名单中的每一个要删除的键
        KEYS_TO_DELETE.forEach(keyToDelete => {
            // 检查最终的 allTranslations 字典中是否存在这个键
            if (Object.prototype.hasOwnProperty.call(allTranslations, keyToDelete)) {
                delete allTranslations[keyToDelete]; // 如果存在，就删除它
                deleteCount++;
                console.log(`  - 删除了键: '${keyToDelete}'`);
            }
        });
