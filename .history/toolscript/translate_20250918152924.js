...
// --- 配置项 ---
// 源JSON文件路径
const SOURCE_JSON_PATH = path.join(__dirname, '..', 'public', 'output.json');
// 翻译词典文件路径 (步骤一生成的)
const TRANSLATION_MAP_PATH = path.join(__dirname, '..', 'public', 'zh_map.json');
// 翻译后输出的文件路径
const OUTPUT_JSON_PATH = path.join(__dirname, '..', 'public', 'output_zh.json');
...
function translateObject(data, translationMap) {
    ...
    // }
    if (typeof data === 'string') {
    const translation = translationMap[data];
    // --- 核心修改 ---
    // 只有当在词典中找到的翻译结果也是一个字符串时，才进行替换。
    if (typeof translation === 'string') {
        return translation;
    }
    // 否则，返回原始字符串，防止对象替换字符串的错误。
    return data;
    // --- 修改结束 ---
}
