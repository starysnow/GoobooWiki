...
async function main() {
    ...
    for (const filePath of sourceFiles) {
        ...
        try {
            ...
            // 增加非数据对象文件计数
            nonDataObjectCount++;
            isSuccess = false; // 严格来说，跳过也算不完全成功
            errorFiles.push({ file: filePath, reason: `被识别为非数据文件，跳过公式转换` });
            console.log(`\n信息：文件 ${filePath} 被识别为非数据文件，已跳过公式转换。`);
        }
    ...
    console.log(`❌ 公式解析失败 (已翻译但保留JS公式): ${formulaParseErrorCount}`);
    console.log(`🟡 跳过公式转换 (被识别为非数据文件): ${nonDataObjectCount}`);
