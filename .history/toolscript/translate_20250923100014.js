 ...
 */
function translateCompoundWords(content, translations) {
    let newContent = content;
    // 正则表达式查找 currency...Gain 和 currency...Cap 模式
    // 例如：匹配 "currency采矿废料Gain"
    const regex = /currency([a-zA-Z\u4e00-\u9fa5]+)(Gain|Cap)/g;

    newContent = newContent.replace(regex, (match, middleWord, suffix) => {
        // middleWord 是捕获到的中间部分，如 "采矿废料"
        // suffix 是 "Gain" 或 "Cap"
        const suffixTranslation = translations[suffix.toLowerCase()] || (suffix === 'Gain' ? '增益' : '容量');
