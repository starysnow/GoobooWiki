...
/**
 * NEW FUNCTION: Translates text with specific prefixes ignored.
 * e.g., translates "mining_rock" by looking up "rock".
 */
function translatePrefixedText(text, translationKeys, translations) {
    if (PREFIX_LIST.length === 0) return text;

    const prefixGroup = PREFIX_LIST.join('|');
    // Regex to find "prefix_word" where word is a potential key
    const regex = new RegExp(`(?<![a-zA-Z0-9_])(${prefixGroup})_([a-zA-Z0-9_]+)`, 'g');

    return text.replace(regex, (match, prefix, word) => {
        // Check if the "word" part exists in our dictionary
        if (translations[word]) {
            return translations[word];
        }
        // If not found, return the original full string (e.g., "mining_rock")
        return match;
    });
}
